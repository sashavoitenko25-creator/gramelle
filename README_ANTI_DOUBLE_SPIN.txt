...
UPDATE (history phantoms)
--------------------------------
Если после первого фикса в «ИСТОРИЯ» появились лишние цвета:
1. Заново выполни sql/roulette_anti_double_spin.sql (секция DELETE phantoms)
2. Задеплой обновлённый src/lib/server/roulette.ts
   — история теперь только раунды с spin_ends_at (реальный спин)
   — orphan-раунды УДАЛЯЮТСЯ, а не settled с фейковым green

Паравоз считает только твои угаданные ставки — он был прав.
История игр теперь совпадёт с реальными спинами.
